...
--	comp_lorenz: lorenz generic map (N=>7, M=>7, sigma=>3, b=>1, r=>5, time_step=>6) port map (clk=>clk, load=> start, reset => reset, stop=> stop_values, x_out <= data_in);
	comp_lorenz: lorenz generic map (N=>7, M=>25, sigma=>3, b=>1, r=>5, time_step=>6) 
		port map (clk=>clk, 
		start=> load, 
		reset => reset, 
		done_out => s_done, 
...
--						end if;
                ...
                when LOAD_ARRAY =>
                    ...
                    values_random(index) <= signed(data_in(23 downto 0));
						  sbox(index) <= to_unsigned(index, 8);
--						  stop_values <= '0';
						  
						  ------------- test ------------
--						  values_out <= signed(data_in);
--						  sbox_out <= to_unsigned(index, 8);
						  -------------------------------
                    if index < 255 then
                        index <= index + 1;
								state <= LOAD_ARRAY;
...
--								counter <= 1;
               ...
               when TEST =>
--						test_out <= '1';
						values_out <= values_random(counter+1);
						sbox_out <= sbox(counter+1);
						if counter < 255 then
							counter <= counter + 1;
                     state <= TEST;
							-----------------
