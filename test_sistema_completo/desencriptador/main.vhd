...
port (
		...
		---------- DEBUG ----------
		SET_MODE		: in std_logic; -- =0 --> cipher // =1 --> inv_cipher
		state_out : out std_logic_vector(3 downto 0);
		FLAG_KEY_READY, FLAG_DATA_READY, FLAG_SEND, FLAG_SBOX_GEN , FLAG_INVALID_DATA  : out std_logic;
		done_key_128_out, done_key_192_out, done_key_256_out, done_lorenz_out, done_mlog_out, done_rossler_out, done_sbox_out, done_cipher_out : out std_logic
...
component mux_generic is
	...
	signal write_data_reg, start_cipher, done_cipher, done_inv_cipher : std_logic;
	signal data_reg_add : integer range 0 to 15;
	signal AES_version  : std_logic_vector(1 downto 0);
	signal plain_text, ciphertext, data_block, data_send : std_logic_vector(127 downto 0);
	...
	signal round_key128_addr, round_key192_addr, round_key256_addr, round_key_addr_c, round_key_addr_ic, round_key_lorenz_addr, round_key_rossler_addr, round_key_mlog_addr, key_round_out : integer range 0 to 15;
	signal round_key_addr : std_logic_vector(3 downto 0);
	...
	signal sbox_addr_key128, sbox_addr_key192, sbox_addr_key256, sbox_addr_cipher, sbox_addr_inv_cipher : std_logic_vector(7 downto 0);
	signal sbox_out, inv_sbox_out,  sbox_out_AES, inv_sbox_out_AES, sbox_out_caotica, inv_sbox_out_caotica, sbox_send : std_logic_vector(7 downto 0);
	signal sbox_mux : std_logic_vector(7 downto 0);
	signal sbox_mux_sel : std_logic_vector(1 downto 0);
	signal sbox_enable_key128, sbox_enable_key192, sbox_enable_key256, sbox_enable_cipher, sbox_enable_inv_cipher : std_logic;
	...
	signal sel_inv_sbox_addr : std_logic_vector(1 downto 0);
	...
	signal enable_reg_out, enable_reg_out_inv : std_logic;
	signal deco_out : std_logic_vector(3 downto 0);
	signal counter : std_logic_vector(1 downto 0);
	signal mux_out_addr_sbox, mux_out_addr_inv_sbox : std_logic_vector(7 downto 0);
...
--	block_counter_out <= block_counter;
	...
	data_register: register_bank generic map(N => 16, M => 8) port map(clk => clk, we => write_data_reg, w_addr => data_reg_add, data_in => RX_byte, data_out => data_block);
	...
	sel_block => sel_block,
	start_cipher => start_cipher, start_key_expance => start_key_expance, done_cipher => (done_cipher and done_inv_cipher), done_key => (done_key_128 and done_key_192 and done_key_256 and done_mlog and done_lorenz and done_rossler),
	done_sbox => done_sbox,
	...
	----------------INSTANCIO CIPHER-------------------
	cipher: main_AES port map(
	clk => clk, reset => reset,
	AES_version => AES_version,
	start => start_cipher and (not SET_MODE),
	done_o => done_cipher,
	
	plain_text => data_block,
	key => key(255 downto 128),
	round_key => round_key,
	round_out => round_key_addr_c,
	...
	----------------INSTANCIO INV CIPHER---------------
	inv_cipher: main_inv_AES port map(
	clk => clk, reset => reset,
	AES_version => AES_version,
	start => start_cipher and SET_MODE,
	done_o => done_inv_cipher,
	
	ciphertext_in => data_block,
	key => key(255 downto 128),
	round_key => round_key,
	round_out => round_key_addr_ic,
	
	sbox_addr => sbox_addr_inv_cipher,
	sbox_out => inv_sbox_out,
	enable_sbox => sbox_enable_inv_cipher,
	
	plaintext_out => plain_text,
	reg_out => enable_reg_out_inv
	);
...
--	key_enable => key_enable_mlog,
	...
	mux_cipher_TX: mux2to1 generic map(M => 128) port map(sel => SET_MODE, d0 => ciphertext, d1 => plain_text, y => data_send); 
...
--	mux_round_key_addr_2: mux2to1 generic map(M => 4) port map (sel => sel_block(2), d0 => out_mux_round, d1 => std_logic_vector(to_unsigned(key_round_out, 4)), y => out_mux_round_2);
	...
	mux_round_key: mux2to1 generic map(M=>4) port map(sel => SET_MODE, d0 => std_logic_vector(to_unsigned(round_key_addr_c, 4)), d1 => std_logic_vector(to_unsigned(round_key_addr_ic, 4)), y => round_key_addr); 
	...
	round_key_addr &																--sel = 110		señal que viene del encriptador para hacer XOR con cada round_key 
